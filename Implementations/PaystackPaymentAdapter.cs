...
{
    public class PaystackPaymentAdapter : IPaymentGatewayAdapter
    {
        private readonly PaymentGatewayProviderConfig _config;

        public PaystackPaymentAdapter(PaymentGatewayProviderConfig config)
        {
            _config = config;
        ...
        public async Task<Result<string>> PayWithCard(CardPaymentModel model, PaymentGatewayProvider? provider = null)
        {
            ...
            // simulate api call to Paystack payment gateway
            await Task.Delay(TimeSpan.FromSeconds(3));

            return Result<string>.Success(Guid.NewGuid().ToString(), "Payment successfully completed with Paystack");
        }
    }
}